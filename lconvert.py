...
def main():
  ...
  parser.add_argument('-q',  dest = 'QM', choices = ["HF", "MP2", "B3LYP", "wB97XD", "CCSD_T"], default="HF")
        ...
        if qm == "MP2":
          ...
          if jt.upper() == "CBS" and bf.upper()=="AUG-CC-PVTZ":
            if not bsse:
              ...
              fout.write("energy('mp2/aug-cc-pv[tq]z', bsse='%s')\n"%bsse)
          if jt.upper() == "CBS" and bf.upper()=="CC-PVTZ":
            if not bsse:
              fout.write("energy('mp2/cc-pv[tq]z')\n")
            else:
              fout.write("energy('mp2/cc-pv[tq]z', bsse='%s')\n"%bsse)
          if jt.upper() == "SP":
            if not bsse:
              fout.write("energy('mp2/%s')\n"%bf)
            else:
              fout.write("energy('mp2/%s', bsse='%s')\n"%(bf,bsse))
        if qm == "CCSD(T)":
          fout.write("mp2_type DF\ne_convergence 7\nreference rhf\n}\n\n")
          if jt.upper() == "CBS":
            if not bsse:
              ...
              fout.write("energy(cbs, corl_wfn='mp2', corl_basis='aug-cc-pv[tq]z', delta_wfn='ccsd(t)', delta_basis='aug-cc-pv[dt]z', bsse='%s')\n"%bsse)
          if jt.upper() == "SP":
            if not bsse:
              fout.write("energy('ccsd(t)/%s')\n"%bf)
            else:
              fout.write("energy('ccsd(t)/%s', bsse='%s')\n"%(bf, bsse))
      return
