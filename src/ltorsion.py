...
def DataPrep():
  ...
  for tor in uniqueTorlistAtom:
    ...
    for ang in range(tor0-start, tor0+start+1, interval):
      ...
      tor.writeQM()
      if ang >= 0:
        fname = "TOR-%s-ANG_+%03d.log"%(torstring,ang)  
      else:
        fname = "TOR-%s-ANG_-%03d.log"%(torstring,abs(ang))
...
def main():
  ...
  parser.add_argument('-rig', dest = 'rigidfit',  default=0,              type=int, help="Use QM geometry in torsion fitting? 0 for NO; 1 for YES")
